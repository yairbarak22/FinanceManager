...
export default function MessageThreadPage({
    ...
    return (
      <div className="p-6">
        <div className="animate-pulse space-y-4">
          ...
          <div className="h-32 bg-[#F7F7F8] rounded-3xl" />
          <div className="h-48 bg-[#F7F7F8] rounded-3xl" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      {/* Subject */}
      <h1 className="text-2xl font-bold text-[#303150] mb-6">{message.subject}</h1>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-[0_4px_20px_rgba(0,0,0,0.08)]">
        <h3 className="text-sm font-semibold text-[#303150] mb-4">
          הגב להודעה
        </h3>
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-2.5 rounded-xl border border-[#E8E8ED] text-sm text-[#303150] hover:border-[#69ADFF] transition-colors"
            >
              <span>
                myneto &lt;{replyFromAddress}&gt;
              </span>
              <ChevronDown
                className={`w-4 h-4 text-[#7E7F90] transition-transform ${
                  showSenderDropdown ? 'rotate-180' : ''
                }`}
              />
            </button>
            {showSenderDropdown && (
              <div className="absolute top-full right-0 left-0 mt-1 bg-white rounded-xl border border-[#E8E8ED] shadow-lg z-10">
                {SENDER_ADDRESSES.map((sender) => (
                  ...
                  >
                    ...
                    <span className="text-[#7E7F90] text-xs mr-2">
                      {sender.email}
                    </span>
                  </button>
                ))}
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="bg-[#69ADFF] hover:bg-[#5A9EE6] disabled:opacity-50 text-white px-6 py-2.5 rounded-xl font-medium text-sm transition-colors flex items-center gap-2 shadow-sm"
          >
            {sending ? (
              <>
