...
interface KnowledgeHeroProps {
  ...
  /** When true, uses compact padding (inside AppLayout) */
  embedded?: boolean;
}

export default function KnowledgeHero({ query, onQueryChange, embedded }: KnowledgeHeroProps) {
  const shouldReduceMotion = useReducedMotion();
  const noMotion = !!shouldReduceMotion;

  return (
    <section
      className={embedded ? 'pt-6 pb-6 md:pt-8 md:pb-8 relative overflow-hidden' : 'pt-28 pb-10 md:pt-32 md:pb-14 relative overflow-hidden'}
      style={{
        background: embedded
    ...
    >
      <div className="relative max-w-3xl mx-auto px-4 sm:px-6 text-center">
        ...
        {/* Subtitle */}
        <motion.p
          className={`${embedded ? 'text-sm mb-5' : 'text-base sm:text-lg mb-8'} max-w-lg mx-auto leading-relaxed`}
          style={{ color: '#86868B', fontFamily: 'var(--font-heebo)' }}
          initial={noMotion ? undefined : { opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, delay: 0.05 }}
        >
          מדריכים פרקטיים על המערכת וידע פיננסי שיעזור לך לקבל החלטות חכמות יותר
        </motion.p>
