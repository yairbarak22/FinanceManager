...
export default function CourseExperience({ course, variant, storageKey }: CourseExperienceProps) {
  ...
  useEffect(() => {
    if (activeLesson?.videoUrl) {
      trackVideoLessonViewed(activeLesson.id, activeLesson.title, activeChapter.title, currentLessonIndex, course.id);
    }
  ...
  return (
    <div className="flex -mt-4 lg:-mt-6 -mb-4 lg:-mb-6 -ms-4 lg:-ms-6 -me-4 lg:-me-6 min-h-[calc(100vh-4rem)]">
      <CourseChapterAccordion
        ...
        playlistTitle={isInvestments ? 'תוכן הקורס' : 'תוכן ההדרכה'}
        showPartnerCTA={isInvestments}
      />
