...
export default function ContactModal({ isOpen, onClose }: ContactModalProps) {
  ...
  const handleSubmit = async () => {
    if (!message.trim()) {
      setError('נא לכתוב את השאלה שלך');
      return;
    }
    ...
    try {
      const res = await apiFetch('/api/investments/contact', {
        method: 'POST',
      ...
      if (!res.ok) {
        // Display server-provided error message (rate limiting, validation, etc.)
        throw new Error(data.error || 'שגיאה בשליחה');
      }

      setSent(true);
      // Keep success state visible for 3 seconds before closing
