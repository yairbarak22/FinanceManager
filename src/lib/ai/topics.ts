...
`,
  ...
  investment_strategy: {
    ...
    title: 'אסטרטגיית השקעה',
    description: 'אסטרטגיה להשקעה עצמאית ב-S&P 500',
    educationalContext: `
אתה מסביר אסטרטגיית השקעה בסיסית למשקיע מתחיל:

מהי אסטרטגיית S&P 500?
- השקעה באינדקס S&P 500 (500 החברות הגדולות בארה"ב)
- גישה פסיבית: קנייה והחזקה לטווח ארוך
- תשואה היסטורית: כ-10% בממוצע שנתי (50 שנה אחרונות)

למה S&P 500 ולא קופות גמל?
- עמלות: 0% במקום 0.7% בקופת גמל
- שליטה מלאה: אתה מנהל את הכסף
- נזילות: אפשר למשוך בכל רגע ללא קנסות
- שקיפות: רואה את התיק בזמן אמת

עקרון Dollar Cost Averaging (DCA):
- השקעה קבועה כל חודש
- מפחית סיכון של "טיימינג" לא נכון
- ממצע עלויות - קונה יותר יחידות כשזול, פחות כשיקר
- דוגמה: 1,000₪ כל חודש ל-VOO או SPY

כללי זהב:
✓ השקע רק כסף שלא תצטרך ל-5+ שנים
✓ אל תמכור בפאניקה בירידות
✓ המשך להשקיע גם בשוק יורד
✓ סבלנות = המפתח להצלחה
`,
    suggestedQuestions: [
      'מה זה S&P 500?',
      'למה השקעה עצמאית עדיפה על קופת גמל?',
      'מה זה Dollar Cost Averaging?',
      'כמה כדאי להשקיע כל חודש?',
      'מה אני קונה בדיוק?',
      'מתי אפשר למכור?',
      'האם זה בטוח?',
      'מה קורה בירידות בשוק?',
    ],
  },

  // ============ המסחר הראשון ============
  first_trade_guide: {
    id: 'first_trade_guide',
    title: 'המסחר הראשון',
    description: 'מדריך לביצוע עסקה ראשונה',
    educationalContext: `
אתה מדריך למשקיע מתחיל איך לבצע עסקת קנייה ראשונה:

צעד 1: חיפוש הנייר
- היכנס לפלטפורמת המסחר של IBI
- חפש בשורת החיפוש: "VOO" או "SPY"
- VOO = Vanguard S&P 500 ETF (עמלות נמוכות יותר)
- SPY = SPDR S&P 500 ETF (נזילות גבוהה יותר)

צעד 2: הזנת פקודת קנייה
- בחר "קנה" (Buy)
- הזן כמות יחידות או סכום בדולרים
- סוג פקודה: LMT (Limit Order) - מומלץ למתחילים!

מהי פקודת Limit (LMT)?
- קונה רק במחיר שקבעת או נמוך יותר
- מגן מפני תנודות פתאומיות
- דוגמה: מחיר נוכחי 500$, שים limit 501$ (מרווח קטן)
- הפקודה תמתין עד שתתמלא במחיר הרצוי

סוגי פקודות נוספות:
- MKT (Market): קונה מיד במחיר השוק (פחות מומלץ)
- STP (Stop): קונה רק אם המחיר עובר סף מסוים

טיפים חשובים:
✓ התחל בסכום קטן (500-1000₪) לניסיון
✓ השתמש ב-LMT תמיד בהתחלה
✓ בדוק שיש מספיק כסף בחשבון המסחר
✓ הפקודה תופיע תחת "פקודות פתוחות"
✓ ברגע שמתמלאת תראה את המניה ב"תיק האחזקות"
`,
    suggestedQuestions: [
      'איך מוצאים את VOO?',
      'מה ההבדל בין VOO ל-SPY?',
      'מה זה פקודת Limit?',
      'איזה סוג פקודה להשתמש?',
      'כמה יחידות לקנות?',
      'איך יודעים שהפקודה התמלאה?',
      'מה אם הפקודה לא מתמלאת?',
      'איפה רואים את האחזקות?',
    ],
  },
  ...
  ibi_transfer: {
    id: 'ibi_transfer',
    title: 'פתיחת חשבון IBI',
    description: 'תהליך פתיחת חשבון והעברת כספים',
    educationalContext: `
אתה מסביר את תהליך פתיחת חשבון ב-IBI והעברת כספים:

תהליך פתיחת חשבון (5-10 דקות):
1. כניסה לקישור ההרשמה של IBI
2. מילוי פרטים אישיים (שם, ת.ז., כתובת)
3. צילום תעודת זהות דרך הטלפון
4. חתימה דיגיטלית על מסמכים
5. אישור החשבון תוך 24 שעות

מה קורה אחרי פתיחת החשבון?
- מקבלים אישור במייל
- נפתח חשבון מסחר (Trading Account)
- צריך להעביר כסף מהבנק כדי להתחיל

העברת כסף מהבנק ל-IBI:
שיטה 1: העברה בנקאית (המומלץ)
- היכנס לאפליקציית הבנק שלך
- בחר "העברת כספים" → "העברה לחשבון אחר"
- פרטי המוטב: חשבון המסחר של IBI (יישלח במייל)
- זמן העברה: 1-3 ימי עסקים
- ללא עמלה בדרך כלל

כמה כסף להעביר בהתחלה?
- מומלץ להתחיל עם 1,000-5,000₪
- אפשר לבדוק את המערכת קודם עם סכום קטן
- אחר כך להעביר סכומים גבוהים יותר

ההטבה: 300₪ מתנה + שנה ללא דמי טיפול!
- מתנה של 300₪ נזקפת לחשבון אחרי הפתיחה
- שנה ראשונה ללא דמי טיפול חודשיים
- חיסכון משמעותי לשנה הראשונה

טיפים:
✓ שמור את כל המיילים מ-IBI
✓ העבר סכום ראשוני קטן לבדיקה
✓ בדוק שהכסף הגיע לפני המסחר הראשון
`,
    suggestedQuestions: [
      'כמה זמן לוקח לפתוח חשבון?',
      'אילו מסמכים צריך?',
      'איך מעבירים כסף מהבנק?',
      'כמה זמן לוקח העברה?',
      'יש עמלה על העברת כסף?',
      'מה המתנה של 300₪?',
      'מה זה דמי טיפול?',
      'אפשר למשוך כסף בחזרה?',
    ],
  },
};
